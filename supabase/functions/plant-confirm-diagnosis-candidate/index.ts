// Confirm an existing diagnosis candidate for a case (owner only; never creates rows or accepts names).
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const plantCaseId = String(body?.caseId || '');
    const diagnosisId = String(body?.diagnosisId || '');
    const uuid = /^[0-9a-f-]{36}$/i;
    if (!uuid.test(plantCaseId) || !uuid.test(diagnosisId)) return jsonResponse({ error: 'invalid_input' }, 400);
