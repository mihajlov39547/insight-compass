...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const goalDirective = (() => {
      switch (pc.user_goal) {
        case 'identify':
          if (true) return 'If plantResearch is present, treat it as PRIMARY grounding for taxonomy, verification, distinguishing features, similar/confusable species, habitat, distribution, and uses/cautions questions: summarise and build on it and cite its web sources by title or domain. If plantResearch is null (notes.noPlantResearch is set), you may mention that deeper plant research has not been run yet and can be started from the case chat header, then still answer from the confirmed identification and Trefle. ' +
