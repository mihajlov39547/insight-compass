ALTER TABLE public.resource_links
  ALTER COLUMN status SET DEFAULT 'linked';

ALTER TABLE public.resource_links
  ADD COLUMN IF NOT EXISTS adapter_key text,
  ADD COLUMN IF NOT EXISTS media_video_id text,
  ADD COLUMN IF NOT EXISTS media_channel_name text,
...
SET
  status = CASE
    WHEN status IN ('ready', 'completed', 'uploaded') THEN 'metadata_ready'
    WHEN status IS NULL OR btrim(status) = '' THEN 'linked'
    ELSE status
  END,
  transcript_status = CASE
    WHEN transcript_status IS NULL OR btrim(transcript_status) = '' THEN 'none'
...
DECLARE
  ...
  v_match text[];
BEGIN
  v_url := public.normalize_resource_url(p_url);

  IF v_url IS NULL OR v_url = '' THEN
    RETURN NULL;
  END IF;

  v_match := regexp_match(v_url, 'youtu\\.be/([A-Za-z0-9_-]{6,})');
  IF v_match IS NOT NULL THEN
    RETURN v_match[1];
  END IF;

  v_match := regexp_match(v_url, '[?&]v=([A-Za-z0-9_-]{6,})');
  IF v_match IS NOT NULL THEN
    RETURN v_match[1];
  END IF;

  v_match := regexp_match(v_url, 'youtube\\.com/(shorts|embed)/([A-Za-z0-9_-]{6,})');
  IF v_match IS NOT NULL THEN
    RETURN v_match[2];
  END IF;
      ...
      metadata = COALESCE(rl.metadata, '{}'::jsonb)
        || jsonb_build_object(
          ...
          'enrichment', jsonb_build_object(
            ...
            'provider', 'youtube',
            'lifecycle', jsonb_build_array('linked', 'metadata_ready', 'transcript_ready')
          )
        )
    WHERE rl.id = v_link.id;
  END IF;
...
AS $$
  ...
  link_resources AS (
    SELECT
      ...
      rl.updated_at,
      COALESCE(NULLIF(rl.status, ''), 'linked') AS processing_status,
